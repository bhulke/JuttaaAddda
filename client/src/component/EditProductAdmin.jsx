                        ...
                        <form className='grid gap-4' onSubmit={handleSubmit}>
                            <div className='grid gap-1'>
                                ...
                                <input id='name' type="text" name='name' placeholder='Enter Product Name' value={data.name} onChange={handleChange} required
                                    className='bg-blue-50 p-2 outline-none border focus-within:border-gray-800 rounded border-blue-200'
                                />
                            </div>
                            <div className='grid gap-1'>
                                ...
                                <textarea id='description' type="text" name='description' placeholder='Enter Product Descriptions' value={data.description} onChange={handleChange} required multiple rows={3}
                                    className='bg-blue-50 p-2 outline-none border focus-within:border-gray-800 rounded resize-none border-blue-200'
                                />
                            </div>
                            <div className='grid gap-1'>
                                    ...
                                    {data.image.length > 0 &&
                                        data.image.map((img, index) => (
                                            <div key={index} className="h-20 w-20 bg-blue-50 border relative group">
                                                ...
                                                <div onClick={() => handleDelImg(index)} className='absolute bottom-0 right-0 p-1 bg-gray-500 hover:bg-gray-600 hover:text-white cursor-pointer rounded-full hidden group-hover:block'>
                                                    <MdDelete />
                                                </div>
                                            </div>
                                        ))}
                                            ...
                                            data.category.map((cat, index) => {
                                                return (
                                                    <div key={cat._id + index + "product"} className='text-sm flex items-center gap-1 bg-blue-50 mt-1'>
                                                        <p>{cat?.name}</p>
                                                        <div className='hover:text-gray-800 cursor-pointer' onClick={() => handleDelCat(index)}>
                                            ...
                                            data.subCategory.map((sub, index) => {
                                                return (
                                                    <div key={sub._id + index + "cat"} className='text-sm flex items-center gap-1 bg-blue-50 mt-1'>
                                                        <p>{sub?.name}</p>
                                                        <div className='hover:text-gray-800 cursor-pointer' onClick={() => handleDelSubCat(index)}>
                                                            <IoMdCloseCircle size={15} />
                                                        </div>
                                                    </div>
                                                )
                                            ...
                                            data.restaurant.map((restro, index) => {
                                                return (
                                                    <div key={restro._id + index + "restro"} className='text-sm flex items-center gap-1 bg-blue-50 mt-1'>
                                                        <p>{restro?.name}</p>
                                                        <div className='hover:text-gray-800 cursor-pointer' onClick={() => handleDelRestro(index)}>
                                                            <IoMdCloseCircle size={15} />
                                                        </div>
                                                    </div>
                                                )
                            ...
                            <div onClick={() => setOpenMoreField(true)} className='inline-block bg-white hover:bg-gray-800 hover:text-white cursor-pointer text-black py-1 px-3 rounded w-32 text-center font-semibold border border-gray-800'>
                                Add Field
                            </div>
                            <button className='bg-gray-800 hover:bg-gray-600 text-white rounded py-2 font-semibold'>
                                Update
                            </button>
                        </form>
                    </div>
