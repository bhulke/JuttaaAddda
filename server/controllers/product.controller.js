...
export const AddProductController = async (req, res) => {
    try {
        const { name, image, category, subCategory, unit, stock, size, price, discount, description, more_details } = req.body

        if (!name || !image[0] || !category[0] || !subCategory[0] || !unit || !size || !stock || !price || !discount || !description) {
...
export const getProductDetails = async (req, res) => {
    try {
        ...
        const product = await ProductModel.findOne({ _id: productId }).populate('name')
